...
class MatchGame extends Component {
  ...
  checkThumbnailImgToMatchImg = id => {
    ...
    const filteredImgItem = imagesList.filter(
      eachImageItem => eachImageItem.id === id,
    )

    if (activeImgId === filteredImgItem[0].id) {
      this.setState(prevState => ({
  ...
  responsiveSection = () => {
    const {activeImgId, activeTabId} = this.state

    const matchImgItem = imagesList.filter(
      eachImgItem => eachImgItem.id === activeImgId,
    )
    const matchImgUrl = matchImgItem[0].imageUrl
    ...
    return (
      <>
        <img src={matchImgUrl} alt="match" className="match-img" />
        ...
        <ul className="thumbnails-container">
          {filteredImagesList.map(eachImage => (
            <ThumbnailItem
              ...
              key={eachImage.id}
              checkThumbnailImgToMatchImg={this.checkThumbnailImgToMatchImg}
            />
          ))}
        </ul>
      </>
  ...
  render() {
    const {timeElapsedInSeconds, isGameOver, score} = this.state
